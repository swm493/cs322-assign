...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (char, freq) :: rest =>
      ...
      val charCombinations = for (i <- 1 to freq) yield (char, i)

      restCombinations ++ (
        for {
          restComb <- restCombinations
          charComb <- charCombinations
        } yield charComb :: restComb
        )
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    x.map { case (char, freq) =>
      (char, freq - yMap.getOrElse(char, 0))
    ...
    def findAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
        ...
        for {
          ...
          restSentence <- findAnagrams(subtract(occurrences, combo))
        } yield {
          word :: restSentence
        }
      }
