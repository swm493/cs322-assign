...
object FunSets {
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a < -bound) true // Lower bound reached
      else if (contains(s, a) && !p(a)) false
      else iter(a-1)
    }
    iter(bound) // Iterate from upper bound to lower bound
  }
   ...
   * that satisfies `p`.
   */
  def exists(s: Set, p: Int => Boolean): Boolean = !forall(s, (src : Int) => !p(src))

  /**
   * Returns a set transformed by applying `f` to each element of `s`.
   */
  def map(s: Set, f: Int => Int): Set = (elem : Int) => exists(s, (src : Int) => f(src) == elem)
